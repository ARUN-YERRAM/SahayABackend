...
/**
 * 📌 API to Schedule Notifications (Stores in Firebase)
 */
app.post("/set-notification", async (req, res) => {
    const { phoneNumber, message, time } = req.body;
...
/**
 * 🔄 Function to Send Scheduled Messages (Runs Every Minute)
 */
exports.sendScheduledMessages = functions.pubsub.schedule("every 1 minutes").timeZone("Asia/Kolkata").onRun(async (context) => {
    const now = new Date();
    const currentTime = now.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" });
    ...
    try {
        ...
        for (const userId of Object.keys(notifications)) {
            const { phoneNumber, message, time } = notifications[userId];
